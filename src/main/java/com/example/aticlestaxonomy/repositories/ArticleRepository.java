...
public interface ArticleRepository extends CrudRepository<Article, Long> {
	...
	@Query(value = "SELECT articles.* FROM articles, article_join_category, categories WHERE `is_category_set`='1' AND rss_feed_id IN :rssFeedList and article_join_category.article_id=articles.id AND article_join_category.category_id=categories.id AND categories.category in :categories LIMIT :limit", nativeQuery = true)
	List<Article> findByCategoryAndRssFeed(@Param("categories") List<String> categories, @Param("rssFeedList") List<Integer> rssFeedList, @Param("limit") int limit);
}
